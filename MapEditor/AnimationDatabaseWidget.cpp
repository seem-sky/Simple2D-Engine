...
void AnimationDatabaseWidget::_onDrop(AnimationView *pTarget, uint32 uiID, Point<int32> pos)
{
    ...
    QPixmap pixmap;
    if (!m_pDBChanger || !m_pSpriteDB->getPrototype(uiID, proto))
        return;
...
bool AnimationDatabaseWidget::getPrototypeFromWidgets(AnimationPrototypePtr &proto)
{
    if (!m_pDBChanger || !m_pDBChanger->getPrototype(getCurrentID(), proto))
        return false;
    if (!DatabaseWidget::getPrototypeFromWidgets(proto) || !m_pAniViewer->scene())
        return false;
