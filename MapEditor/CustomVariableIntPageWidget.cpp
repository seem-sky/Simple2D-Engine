...
void CustomVariableIntPageWidget::SelectItem(uint32 p_uiID)
{
    ClearWidgets();
    DisconnectWidgets();
    if (DatabaseWindow *t_pDBWindow = (DatabaseWindow*)window())
    {
        ...
        ObjectIntegerList::const_iterator t_Itr = t_Proto->m_ObjectIntegerList.find(p_uiID);
...
void CustomVariableIntPageWidget::ChangeItem(uint32 p_uiID, bool p_bDelete)
{
...
void CustomVariableIntPageWidget::ClickNew()
{
    if (DatabaseWindow *t_pDBWindow = (DatabaseWindow*)window())
    {
        if (DatabaseOutput *t_pDB = DatabaseOutput::Get())
