...
void CustomVariableFloatPageWidget::ChangeItem(uint32 p_uiID, bool p_bDelete /* = false */)
{
    if (!m_pVariableHolder)
        return;

    m_pVariableHolder->SetFloat(p_uiID, GetVariableFromData());
}

void CustomVariableFloatPageWidget::SelectItem(uint32 p_uiID, uint32 p_uiParentID /* = 0 */)
{
    ClearWidgets();
    DisconnectWidgets();
    uint32 t_uiID = 0;
    ...
    const TVariable<float> *t_pVar = t_pVarHolder->GetFloat(p_uiID);
    if (!t_pVar)
        return;
...
void CustomVariableFloatPageWidget::LoadItems()
{
        ...
        if (DatabaseOutput *t_pDBOut = DatabaseOutput::Get())
        {
            ...
            for (IDList::iterator t_Itr = t_uiParentIDList.begin(); t_Itr != t_uiParentIDList.end(); ++t_Itr)
            {
                if (const VariableHolder *t_pVarHolder = t_pDBOut->GetVariableHolderFromParent(m_pPrototype->m_PrototypeType, *t_Itr))
                {
                    ...
                    t_pVarHolder->GetFloatNames(t_VarNames);
    ...
    std::vector<std::string> t_VarNames;
    m_pVariableHolder->GetFloatNames(t_VarNames);
    for (uint32 i = 0; i < t_VarNames.size(); ++i)
        m_pStoreBox->addItem(QString::number(i)+":" + QString::fromStdString(t_VarNames.at(i)));
}

void CustomVariableFloatPageWidget::ResizeVariableCount(uint32 p_uiCount)
{
    if (!m_pVariableHolder)
        return;

    uint32 t_uiCurCount = m_pVariableHolder->GetFloatCount();
    if (t_uiCurCount < p_uiCount)
        m_pVariableHolder->SetFloatCount(p_uiCount);
...
uint32 CustomVariableFloatPageWidget::GetVariableCount()
{
    if (!m_pVariableHolder)
        return 0;

    return m_pVariableHolder->GetFloatCount();
}