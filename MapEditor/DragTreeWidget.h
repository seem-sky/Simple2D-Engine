...
public:
    ...
    inline void setDB(const boost::shared_ptr<const DATABASE::DatabaseChanger<T>> &pDB) { m_pDB = pDB; fillWithPrototypes(); }

protected:
    boost::shared_ptr<const DATABASE::DatabaseChanger<T>> m_pDB;
};
typedef PrototypeTreeWidget<DATABASE::ObjectAnimationTypePrototype> ObjectAnimationTypeTreeWidget;

/*#####
# AnimationPrototypeDragTreeWidget
#####*/
class AnimationPrototypeDragTreeWidget : public PrototypeTreeWidget<DATABASE::AnimationPrototype>
{
protected:
...
#####*/
template<class T>
class TexturePrototypeDragTreeWidget : public PrototypeTreeWidget<T>
{
protected:
    void startDrag(Qt::DropActions supportedActions)
    {
        if (QTreeWidgetItem *pItem = currentItem())
        {
            ...
            if (m_pDB && m_pDB->getPrototype(pItem->text(0).toUInt(), proto) && createPixmapFromTexturePrototype(proto, pixmap))
            {
    ...
    QWidget* setTooltipWidget(uint32 uiPrototypeID)
    {
        boost::shared_ptr<const T> proto;
        QPixmap pixmap;
        if (m_pDB && m_pDB->getPrototype(uiPrototypeID, proto) && createPixmapFromTexturePrototype(proto, pixmap))
        {
...
#####*/
class WorldObjectPrototypeDragTreeWidget : public PrototypeTreeWidget<DATABASE::WorldObjectPrototype>
{
    Q_OBJECT
protected:
    QWidget* setTooltipWidget(uint32 uiPrototypeID);
