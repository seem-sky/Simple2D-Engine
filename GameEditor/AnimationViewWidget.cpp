...
{
    if (m_pAnimationDB && m_pAnimationDB->getItem(uiID, proto))
        return true;
    return false;
}
...
{
    if (m_pSpriteDB && m_pSpriteDB->getItem(uiID, proto))
        return true;
    return false;
}
...
void AnimationViewScene::_drawPreviousFrame(QPainter *painter, const QRectF &rect)
{
    ...
    if (AnimationDatabaseWidget *pWidget = dynamic_cast<AnimationDatabaseWidget*>(parent()->parent()))
    {
        ...
        for (uint32 i = 0; i < m_PreviousFrame.m_Sprites.size(); ++i)
        {
            ...
            if (!pDB->getItem(sprite.m_uiSpriteID, proto) || !createPixmapFromTexturePrototype(proto, pixmap))
                continue;
