...
namespace DATABASE
{
    ...
    const SpritePrototype* Database::GetSpritePrototype(UINT p_uiID)
    {
        SpriteList::const_iterator t_SpriteItr = m_SpriteDB.find(p_uiID);
        if (t_SpriteItr != m_SpriteDB.end())
             ...
             t_DBitr2 != t_DBitr->second.m_ChildList.end() && t_DBitr2->first == "Sprite"; ++t_DBitr2)
        {
            if (t_DBitr2->second.HasAttributes())
            {
                ...
                if (t_AttrItr->second.uintVal == p_uiID)
                {
                    ...
                    m_SpriteDB.insert(std::make_pair(t_proto.m_uiID, t_proto));
                    return &(m_SpriteDB.find(p_uiID)->second);
                }
            }
        }
        return NULL;
